...
//        if (mIsLoop) return;
    ...
    private float computeScale(int x) {
        float scale = 1 - Math.abs(x - mStartX) * 1.0f / Math.abs(mStartX + mDecoratedChildWidth / mZoomRatio);
        if (scale < 0) scale = 0;
        if (scale > 1) scale = 1;
        return scale;
    }
    ...
    private void fixOffsetWhenFinishScroll() {
        if (getIntervalDistance() != 0) { // 判断非 0 ，否则除 0 会导致异常
            ...
            startScroll(mOffsetAll, finalOffset);
            loopCurrentPos = Math.abs(Math.round(finalOffset * 1.0f / getIntervalDistance()));
            Log.e("snowBanner", "=====loopCurrentPos======" + loopCurrentPos);
            mSelectPosition = loopCurrentPos % getItemCount();
        }
    }
    ...
    private void onSelectedCallBack() {
        loopCurrentPos = Math.round(mOffsetAll / getIntervalDistance());
        mSelectPosition = Math.abs(loopCurrentPos % getItemCount());
    ...
    static class Builder {
        ...
        Builder setZoomRatio(float zoomRatio) {
            csZoomRatio = zoomRatio;
            return this;
        }
        ...
        public CoverFlowLayoutManger build() {
            return new CoverFlowLayoutManger(isFlat, isGreyItem,
                    isAlphaItem, cstIntervalRatio, csZoomRatio, isLoop, is3DItem);
        }
    }
}